...
if ! [[ "${BEAM_SIZE}" =~ ^[1-9][0-9]*$ ]]; then
    echo "ERROR: BEAM_SIZE must be a positive integer, got '${BEAM_SIZE}'." >&2
    exit 1
fi
...
if [[ "${INFERENCE_BACKEND}" == "vllm" ]]; then
    ...
    # to force eager mode for environments where compilation is unstable.
    if [[ "${DISPERSION4Q_DISABLE_TORCH_COMPILE:-0}" == "1" ]]; then
        export TORCH_COMPILE_DISABLE=1
        export TORCHDYNAMO_DISABLE=1
    else
        unset TORCH_COMPILE_DISABLE TORCHDYNAMO_DISABLE || true
    fi
fi
...
echo "Resolved model id is set to: $MODEL_ID"
...
echo "Language set is set to: $LANG_SET"
echo "Language pairs are set to: $LANG_PAIRS_CSV"
if [[ "${INFERENCE_BACKEND}" == "vllm" ]]; then
...
echo "Test dataset is set to: $TEST_DATASET"
...
for EPOCH in 0; do
    ...
    if [[ "${INFERENCE_BACKEND}" == "vllm" ]]; then
        ...
        if [[ -n "${QUANTIZATION_CONFIG}" ]]; then
            ...
            HAS_QUANTIZED_MODEL=0
            if [[ -f "${QUANTIZED_MODEL_DIR}/config.json" ]] \
                && grep -q '"quantization_config"' "${QUANTIZED_MODEL_DIR}/config.json" \
                && compgen -G "${QUANTIZED_MODEL_DIR}/*.safetensors" > /dev/null; then
                HAS_QUANTIZED_MODEL=1
            fi

            if [[ "${HAS_QUANTIZED_MODEL}" == "1" ]]; then
                echo "Skipping quantization: found existing quantized model at ${QUANTIZED_MODEL_DIR}"
            else
        ...
        python experiments/inference_formal.py --model_name ${MODEL_ID} \
